...
{
    public abstract class AbstractPreferencesStorage: IAsyncPreferences
    {
        protected virtual bool HasTokenBeenCancelled(CancellationToken? ct)
        {
        ...
        public void Set<T>(string key, T? value, string? sharedName = null) => AsyncHelper.RunSync(() =>  SetAsync(key, value,  sharedName ));
        public async Task<bool> SetAsync<T>(string key, T? value, string? sharedName, CancellationToken? ct = null) => await TryPersistAsync(key, value, ct??CancellationToken.None);
        
        public T? Get<T>(string key, T? defaultValue,  string? sharedName = null) =>  AsyncHelper.RunSync(() =>  GetAsync(key, defaultValue, sharedName));
        public async Task<T?> GetAsync<T>(string key, T? defaultValue, string? sharedName = null, CancellationToken? ct = null) =>
            !ContainsKey(key) ? defaultValue : await LoadAsync<T?>(key, ct ?? CancellationToken.None);

        public void Remove(string key, string? sharedName = null) => AsyncHelper.RunSync(() =>  RemoveAsync(key, sharedName));
